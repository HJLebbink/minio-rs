...
## AWS S3 Tables API Operations

In addition to the Apache Iceberg REST Catalog API, the SDK implements the full AWS S3 Tables API for compatibility with AWS services. These operations are designed for future compatibility when MinIO AIStor adds support.
...
| Policy            | 6          | SDK Ready     |
| Tagging           | 3          | SDK Ready     |
| Encryption        | 6          | SDK Ready     |
| Maintenance       | 5          | SDK Ready     |
| Replication       | 7          | SDK Ready     |
| Storage Class     | 3          | SDK Ready     |
| Metrics           | 3          | SDK Ready     |
| Record Expiration | 3          | SDK Ready     |
| **Total**         | **36**     | **SDK Ready** |

### Policy Operations

Manage access control policies for warehouses (table buckets) and tables.

| Operation             | Method | Path                                               | Description             |
| --------------------- | ------ | -------------------------------------------------- | ----------------------- |
| GetWarehousePolicy    | GET    | `/warehouses/{name}/policy`                        | Get warehouse policy    |
| PutWarehousePolicy    | PUT    | `/warehouses/{name}/policy`                        | Set warehouse policy    |
| DeleteWarehousePolicy | DELETE | `/warehouses/{name}/policy`                        | Delete warehouse policy |
| GetTablePolicy        | GET    | `/warehouses/{w}/namespaces/{n}/tables/{t}/policy` | Get table policy        |
| PutTablePolicy        | PUT    | `/warehouses/{w}/namespaces/{n}/tables/{t}/policy` | Set table policy        |
| DeleteTablePolicy     | DELETE | `/warehouses/{w}/namespaces/{n}/tables/{t}/policy` | Delete table policy     |

### Tagging Operations

Manage resource tags for cost allocation and access control (ABAC).

| Operation           | Method | Path                              | Description                 |
| ------------------- | ------ | --------------------------------- | --------------------------- |
| TagResource         | POST   | `/tags/{resourceArn}`             | Add tags to a resource      |
| UntagResource       | DELETE | `/tags/{resourceArn}?tagKeys=...` | Remove tags from a resource |
| ListTagsForResource | GET    | `/tags/{resourceArn}`             | List tags for a resource    |

### Encryption Operations

Configure server-side encryption for warehouses and tables.

| Operation                 | Method | Path                                                   | Description                        |
| ------------------------- | ------ | ------------------------------------------------------ | ---------------------------------- |
| GetWarehouseEncryption    | GET    | `/warehouses/{name}/encryption`                        | Get warehouse encryption config    |
| PutWarehouseEncryption    | PUT    | `/warehouses/{name}/encryption`                        | Set warehouse encryption config    |
| DeleteWarehouseEncryption | DELETE | `/warehouses/{name}/encryption`                        | Delete warehouse encryption config |
| GetTableEncryption        | GET    | `/warehouses/{w}/namespaces/{n}/tables/{t}/encryption` | Get table encryption config        |
| PutTableEncryption        | PUT    | `/warehouses/{w}/namespaces/{n}/tables/{t}/encryption` | Set table encryption config        |
| DeleteTableEncryption     | DELETE | `/warehouses/{w}/namespaces/{n}/tables/{t}/encryption` | Delete table encryption config     |

### Maintenance Operations

Configure Iceberg table maintenance (compaction, snapshot management, file cleanup).

| Operation                    | Method | Path                                                                  | Description                      |
| ---------------------------- | ------ | --------------------------------------------------------------------- | -------------------------------- |
| GetWarehouseMaintenance      | GET    | `/warehouses/{name}/maintenance`                                      | Get warehouse maintenance config |
| PutWarehouseMaintenance      | PUT    | `/warehouses/{name}/maintenance/{type}`                               | Set warehouse maintenance config |
| GetTableMaintenance          | GET    | `/warehouses/{w}/namespaces/{n}/tables/{t}/maintenance`               | Get table maintenance config     |
| PutTableMaintenance          | PUT    | `/warehouses/{w}/namespaces/{n}/tables/{t}/maintenance/{type}`        | Set table maintenance config     |
| GetTableMaintenanceJobStatus | GET    | `/warehouses/{w}/namespaces/{n}/tables/{t}/maintenance/{type}/status` | Get maintenance job status       |

### Replication Operations

Configure cross-region replication for warehouses and tables.

| Operation                  | Method | Path                                                           | Description                         |
| -------------------------- | ------ | -------------------------------------------------------------- | ----------------------------------- |
| GetWarehouseReplication    | GET    | `/warehouses/{name}/replication`                               | Get warehouse replication config    |
| PutWarehouseReplication    | PUT    | `/warehouses/{name}/replication`                               | Set warehouse replication config    |
| DeleteWarehouseReplication | DELETE | `/warehouses/{name}/replication`                               | Delete warehouse replication config |
| GetTableReplication        | GET    | `/warehouses/{w}/namespaces/{n}/tables/{t}/replication`        | Get table replication config        |
| PutTableReplication        | PUT    | `/warehouses/{w}/namespaces/{n}/tables/{t}/replication`        | Set table replication config        |
| DeleteTableReplication     | DELETE | `/warehouses/{w}/namespaces/{n}/tables/{t}/replication`        | Delete table replication config     |
| GetTableReplicationStatus  | GET    | `/warehouses/{w}/namespaces/{n}/tables/{t}/replication/status` | Get replication status              |
...
| Operation                | Method | Path                                                     | Description                 |
| ------------------------ | ------ | -------------------------------------------------------- | --------------------------- |
| GetWarehouseStorageClass | GET    | `/warehouses/{name}/storageclass`                        | Get warehouse storage class |
| PutWarehouseStorageClass | PUT    | `/warehouses/{name}/storageclass`                        | Set warehouse storage class |
| GetTableStorageClass     | GET    | `/warehouses/{w}/namespaces/{n}/tables/{t}/storageclass` | Get table storage class     |
...
| Operation              | Method | Path                         | Description                     |
| ---------------------- | ------ | ---------------------------- | ------------------------------- |
| GetWarehouseMetrics    | GET    | `/warehouses/{name}/metrics` | Get warehouse metrics config    |
| PutWarehouseMetrics    | PUT    | `/warehouses/{name}/metrics` | Set warehouse metrics config    |
| DeleteWarehouseMetrics | DELETE | `/warehouses/{name}/metrics` | Delete warehouse metrics config |

### Record Expiration Operations

Configure automatic record expiration based on timestamp columns.

| Operation                   | Method | Path                                                          | Description               |
| --------------------------- | ------ | ------------------------------------------------------------- | ------------------------- |
| GetTableExpiration          | GET    | `/warehouses/{w}/namespaces/{n}/tables/{t}/expiration`        | Get expiration config     |
| PutTableExpiration          | PUT    | `/warehouses/{w}/namespaces/{n}/tables/{t}/expiration`        | Set expiration config     |
| GetTableExpirationJobStatus | GET    | `/warehouses/{w}/namespaces/{n}/tables/{t}/expiration/status` | Get expiration job status |

### Type Definitions

The SDK includes complete type definitions for all AWS S3 Tables API operations:

| File                   | Types                                                                          |
| ---------------------- | ------------------------------------------------------------------------------ |
| `types/encryption.rs`  | `EncryptionConfiguration`, `SseAlgorithm`                                      |
| `types/maintenance.rs` | `MaintenanceConfiguration`, `CompactionSettings`, `SnapshotManagementSettings` |
| `types/replication.rs` | `ReplicationConfiguration`, `ReplicationRule`, `ReplicationRuleStatus`         |
| `types/storage.rs`     | `StorageClass`                                                                 |
| `types/metrics.rs`     | `MetricsConfiguration`, `MetricsStatus`                                        |
| `types/expiration.rs`  | `RecordExpirationConfiguration`, `ExpirationStatus`, `ExpirationJobStatus`     |
| `types/tag.rs`         | `Tag`                                                                          |

### Response Traits

The SDK provides response traits for convenient field access:

| Trait                                  | Method                        | Returns                                        |
| -------------------------------------- | ----------------------------- | ---------------------------------------------- |
| `HasResourcePolicy`                    | `resource_policy()`           | `Result<String, Error>`                        |
| `HasTags`                              | `tags()`                      | `Result<Vec<Tag>, Error>`                      |
| `HasEncryptionConfiguration`           | `encryption_configuration()`  | `Result<EncryptionConfiguration, Error>`       |
| `HasWarehouseMaintenanceConfiguration` | `maintenance_configuration()` | `Result<...>`                                  |
| `HasTableMaintenanceConfiguration`     | `maintenance_configuration()` | `Result<...>`                                  |
| `HasMaintenanceJobStatus`              | `job_status()`                | `Result<MaintenanceJobStatus, Error>`          |
| `HasReplicationConfiguration`          | `replication_configuration()` | `Result<ReplicationConfiguration, Error>`      |
| `HasReplicationStatus`                 | `replication_status()`        | `Result<ReplicationStatus, Error>`             |
| `HasStorageClass`                      | `storage_class()`             | `Result<StorageClass, Error>`                  |
| `HasMetricsConfiguration`              | `metrics_configuration()`     | `Result<MetricsConfiguration, Error>`          |
| `HasExpirationConfiguration`           | `expiration_configuration()`  | `Result<RecordExpirationConfiguration, Error>` |
| `HasExpirationJobStatus`               | `expiration_job_status()`     | `Result<ExpirationJobStatus, Error>`           |
...
async fn configure_table_encryption() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let namespace = Namespace::single("my_namespace")?;
    let table = TableName::try_from("my_table")?;

    // Set table encryption to KMS
    let encryption = EncryptionConfiguration::kms_managed(
    ...
    println!("Encryption algorithm: {:?}", config.sse_algorithm());
...
These AWS S3 Tables API operations are **not yet implemented** in MinIO AIStor. The SDK implementations are ready for when server support is added. Calling these operations against MinIO AIStor will return "unsupported API call" errors.

## References

- Apache Iceberg REST Catalog API: https://iceberg.apache.org/spec/#rest-catalog-api
- AWS S3 Tables API: https://docs.aws.amazon.com/AmazonS3/latest/API/API_Operations_Amazon_S3_Tables.html

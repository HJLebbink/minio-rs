    ...
    pub fn validate(&self, bucket_name: &str) -> Result<(), Error> {
        ...
        for resource in &self.resources {
            if !resource.contains(bucket_name) {
                return Err(Error::InvalidBucketPolicy(format!(
...
impl BucketPolicy {
    ...
    pub fn parse_from_json<R: Read>(reader: R, bucket_name: &str) -> Result<Self, Error> {
        let mut policy: BucketPolicy = serde_json::from_reader(reader)?;
        ...
        for rule in &self.rules {
            ...
            for statement in &rule.statements {
                ...
                if let Some(principal) = &statement.principal {
                    ...
                    principal_elem
                        .children
                        .push(XMLNode::Text(principal.clone()));

...
use crate::s3::builders::{PutObject, UploadPart};
...
use crate::s3::types::{BucketName, ETag, ObjectKey, PartInfo, S3Api, ToStream, UploadId};
...
impl MinioMultipartUpload {
    ...
    /// Convert validation error to ObjectStore error.
    fn convert_validation_error(err: crate::s3::error::ValidationErr) -> ObjectStoreError {
        ObjectStoreError::Generic {
            store: "MinIO",
            source: Box::new(err),
        }
...
impl MultipartUpload for MinioMultipartUpload {
    ...
    fn put_part(
        ...
        Box::pin(async move {
            ...
            let response = client
                ...
                .map_err(Self::convert_validation_error)?
                .build()
                .send()
                .await
    ...
    async fn complete(&mut self) -> Result<PutResult> {
        ...
        let e_tag = response.etag().ok().map(|e| e.to_string());
...
impl MinioObjectStore {
    ...
    /// Helper to convert validation errors to ObjectStore errors.
    fn convert_validation_error(err: crate::s3::error::ValidationErr) -> ObjectStoreError {
        ObjectStoreError::Generic {
            store: "MinIO",
            source: Box::new(err),
...
impl ObjectStore for MinioObjectStore {
    ...
    async fn head(&self, location: &Path) -> Result<ObjectMeta> {
        ...
        let e_tag = stat.etag().ok().map(ETag::into_inner);
    ...
    fn list(&self, prefix: Option<&Path>) -> BoxStream<'static, Result<ObjectMeta>> {
        ...
        Box::pin(async_stream::stream! {
            let builder = match client.list_objects(bucket) {
                Ok(b) => b.recursive(true).prefix(prefix_str),
                Err(e) => {
                    yield Err(Self::convert_validation_error(e));
                    return;
                }
            };
                    ...
                    Ok(response) => {
                        for item in response.contents {
                            yield Ok(ObjectMeta {
                                ...
                                e_tag: item.etag.map(|e| e.to_string()),
                                version: None,
                            });
                        }
                    }

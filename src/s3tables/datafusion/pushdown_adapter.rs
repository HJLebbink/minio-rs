...
use crate::s3::builders::UploadPart;
...
impl PushdownMinioObjectStore {
    ...
    fn convert_validation_error(err: crate::s3::error::ValidationErr) -> object_store::Error {
        object_store::Error::Generic {
            store: "MinIO",
            source: Box::new(err),
        }
...
impl ObjectStore for PushdownMinioObjectStore {
    ...
    async fn head(&self, location: &Path) -> Result<ObjectMeta> {
        ...
        let e_tag = stat.etag().ok().map(|e| e.to_string());

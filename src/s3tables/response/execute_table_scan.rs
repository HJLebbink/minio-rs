...
/// CSV data with header row and data rows as key-value maps.
pub type CsvData = (Vec<String>, Vec<HashMap<String, String>>);
...
impl ExecuteTableScanResponse {
    ...
    /// # }
    /// ```
    pub fn csv_rows(&self) -> Result<CsvData, ValidationErr> {
        let text = self.text()?;

...
//! Client method for ListVectors operation.

use crate::s3::error::ValidationErr;
use crate::s3vectors::builders::{ListVectors, ListVectorsBldr};
use crate::s3vectors::client::VectorsClient;
use crate::s3vectors::types::{BucketName, IndexName};

impl VectorsClient {
    /// Creates a builder for the ListVectors operation.
    ///
    /// # Arguments
    ///
    ...
    /// let resp = client
    ///     .list_vectors("my-bucket", "my-index")?
    ///     .build()
    ///     .send()
    ///     .await?;
    /// # Ok(())
    /// # }
    /// ```
    pub fn list_vectors<B, I>(&self, bucket: B, index: I) -> Result<ListVectorsBldr, ValidationErr>
    where

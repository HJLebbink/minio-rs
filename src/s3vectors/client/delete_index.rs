...
//! Client method for DeleteIndex operation.

use crate::s3::error::ValidationErr;
use crate::s3vectors::builders::{DeleteIndex, DeleteIndexBldr};
use crate::s3vectors::client::VectorsClient;
use crate::s3vectors::types::{BucketName, IndexName};

impl VectorsClient {
    /// Creates a builder for the DeleteIndex operation.
    ///
    /// # Arguments
    ///
    ...
    /// let resp = client
    ///     .delete_index("my-bucket", "my-index")?
    ///     .build()
    ///     .send()
    ///     .await?;
    /// # Ok(())
    /// # }
    /// ```
    pub fn delete_index<B, I>(&self, bucket: B, index: I) -> Result<DeleteIndexBldr, ValidationErr>
    where

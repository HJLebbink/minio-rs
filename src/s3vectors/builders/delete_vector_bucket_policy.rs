...
//! Builder for DeleteVectorBucketPolicy operation.

use crate::s3::error::ValidationErr;
use crate::s3vectors::client::VectorsClient;
use crate::s3vectors::error::VectorsValidationErr;
use crate::s3vectors::response::DeleteVectorBucketPolicyResponse;
use crate::s3vectors::types::{
    BucketName, ToVectorsRequest, VectorBucketArn, VectorsApi, VectorsRequest,
};
use typed_builder::TypedBuilder;

/// Builder for the [`DeleteVectorBucketPolicy`](https://docs.aws.amazon.com/AmazonS3/latest/API/API_S3VectorBuckets_DeleteVectorBucketPolicy.html) S3Vectors API operation.
///
...
/// Builder type alias for [`DeleteVectorBucketPolicy`].
pub type DeleteVectorBucketPolicyBldr =
    DeleteVectorBucketPolicyBuilder<((VectorsClient,), (Option<BucketName>,), ())>;

impl VectorsApi for DeleteVectorBucketPolicy {
    type VectorsResponse = DeleteVectorBucketPolicyResponse;
}

impl ToVectorsRequest for DeleteVectorBucketPolicy {
    fn to_vectors_request(self) -> Result<VectorsRequest, ValidationErr> {
        ...
        Ok(VectorsRequest::builder()
            .client(self.client)
            .path("/DeleteVectorBucketPolicy".to_string())
            .body(Some(body))
            .build())
    }
}

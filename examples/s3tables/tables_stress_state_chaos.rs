...
//! 2. Spawn multiple threads performing random operations from 70 different API operations.
//!    Currently 34 operations are enabled; others await server support.
//!
//!    **ENABLED OPERATIONS (34)**
//!
...
//!
//!    Scan Planning Operations (4):
//!    - PlanTableScan, FetchPlanningResult, FetchScanTasks, CancelPlanning
//!
//!    Direct Scan Execution (1):
//!    - ExecuteTableScan
//!
//!    **PENDING SERVER SUPPORT (36)**
//!
//!    Policy Operations (6): Get/Put/Delete for Warehouse and Table policies
//!    Encryption Operations (6): Get/Put/Delete for Warehouse and Table encryption
//!    Maintenance Operations (5): Get/Put for Warehouse/Table maintenance config
//!    Replication Operations (7): Get/Put/Delete/Status for Warehouse/Table replication
//!    Storage Class Operations (3): Get/Put for Warehouse/Table storage class
//!    Metrics Operations (3): Get/Put/Delete for Warehouse metrics config
//!    Expiration Operations (3): Get/Put for Table expiration, job status
//!    Tagging Operations (3): TagResource, UntagResource, ListTagsForResource
//!
//!    To enable these operations, uncomment them in `SUPPORTED_OPERATIONS`
//!
...
use minio::s3tables::utils::{
    Namespace, PlanId as PlanIdType, TableName, ViewName, ViewSql, WarehouseName,
};
...
enum ChaosOperation {
    ...
    FetchScanTasks,
    CancelPlanning,
    // Policy operations - Warehouse (3)
    GetWarehousePolicy,
    PutWarehousePolicy,
    DeleteWarehousePolicy,
    // Policy operations - Table (3)
    GetTablePolicy,
    PutTablePolicy,
    DeleteTablePolicy,
    // Encryption operations - Warehouse (3)
    GetWarehouseEncryption,
    PutWarehouseEncryption,
    DeleteWarehouseEncryption,
    // Encryption operations - Table (3)
    GetTableEncryption,
    PutTableEncryption,
    DeleteTableEncryption,
    // Maintenance operations - Warehouse (2)
    GetWarehouseMaintenance,
    PutWarehouseMaintenance,
    // Maintenance operations - Table (3)
    GetTableMaintenance,
    PutTableMaintenance,
    GetTableMaintenanceJobStatus,
    // Replication operations - Warehouse (3)
    GetWarehouseReplication,
    PutWarehouseReplication,
    DeleteWarehouseReplication,
    // Replication operations - Table (4)
    GetTableReplication,
    PutTableReplication,
    DeleteTableReplication,
    GetTableReplicationStatus,
    // Storage class operations (3)
    GetWarehouseStorageClass,
    PutWarehouseStorageClass,
    GetTableStorageClass,
    // Metrics operations - Warehouse (3)
    GetWarehouseMetrics,
    PutWarehouseMetrics,
    DeleteWarehouseMetrics,
    // Expiration operations - Table (3)
    GetTableExpiration,
    PutTableExpiration,
    GetTableExpirationJobStatus,
...
/// Configuration for which operations are currently supported by the server.
/// Operations not in this list will be skipped during chaos testing.
/// Update this list as new API features are implemented in the server.
const SUPPORTED_OPERATIONS: &[ChaosOperation] = &[
    // Core warehouse operations - SUPPORTED
    ChaosOperation::CreateWarehouse,
    ChaosOperation::DeleteWarehouse,
    ChaosOperation::ListWarehouses,
    ChaosOperation::GetWarehouse,
    // Core namespace operations - SUPPORTED
    ...
    // Core table operations - SUPPORTED
    ...
    // Config & Metrics - SUPPORTED
    ChaosOperation::GetConfig,
    ChaosOperation::TableMetrics,
    // Namespace property operations - SUPPORTED
    ChaosOperation::UpdateNamespaceProperties,
    // Credentials operations - SUPPORTED
    ...
    // Scan planning operations - SUPPORTED
    ChaosOperation::PlanTableScan,
    ChaosOperation::FetchPlanningResult,
    ChaosOperation::FetchScanTasks,
    ChaosOperation::CancelPlanning,
    // Execute table scan - SUPPORTED
    ChaosOperation::ExecuteTableScan,
    // Policy operations - NOT YET SUPPORTED (uncomment when server supports)
    // ChaosOperation::GetWarehousePolicy,
    // ChaosOperation::PutWarehousePolicy,
    // ChaosOperation::DeleteWarehousePolicy,
    // ChaosOperation::GetTablePolicy,
    // ChaosOperation::PutTablePolicy,
    // ChaosOperation::DeleteTablePolicy,
    // Encryption operations - NOT YET SUPPORTED (uncomment when server supports)
    // ChaosOperation::GetWarehouseEncryption,
    // ChaosOperation::PutWarehouseEncryption,
    // ChaosOperation::DeleteWarehouseEncryption,
    // ChaosOperation::GetTableEncryption,
    // ChaosOperation::PutTableEncryption,
    // ChaosOperation::DeleteTableEncryption,
    // Maintenance operations - NOT YET SUPPORTED (uncomment when server supports)
    // ChaosOperation::GetWarehouseMaintenance,
    // ChaosOperation::PutWarehouseMaintenance,
    // ChaosOperation::GetTableMaintenance,
    // ChaosOperation::PutTableMaintenance,
    // ChaosOperation::GetTableMaintenanceJobStatus,
    // Replication operations - NOT YET SUPPORTED (uncomment when server supports)
    // ChaosOperation::GetWarehouseReplication,
    // ChaosOperation::PutWarehouseReplication,
    // ChaosOperation::DeleteWarehouseReplication,
    // ChaosOperation::GetTableReplication,
    // ChaosOperation::PutTableReplication,
    // ChaosOperation::DeleteTableReplication,
    // ChaosOperation::GetTableReplicationStatus,
    // Storage class operations - NOT YET SUPPORTED (uncomment when server supports)
    // ChaosOperation::GetWarehouseStorageClass,
    // ChaosOperation::PutWarehouseStorageClass,
    // ChaosOperation::GetTableStorageClass,
    // Metrics operations - NOT YET SUPPORTED (uncomment when server supports)
    // ChaosOperation::GetWarehouseMetrics,
    // ChaosOperation::PutWarehouseMetrics,
    // ChaosOperation::DeleteWarehouseMetrics,
    // Expiration operations - NOT YET SUPPORTED (uncomment when server supports)
    // ChaosOperation::GetTableExpiration,
    // ChaosOperation::PutTableExpiration,
    // ChaosOperation::GetTableExpirationJobStatus,
    // Tagging operations - NOT YET SUPPORTED (uncomment when server supports)
    // ChaosOperation::TagResource,
    // ChaosOperation::UntagResource,
    // ChaosOperation::ListTagsForResource,
];

impl ChaosOperation {
    fn random() -> Self {
        let mut rng = rand::rng();
        let idx = rng.random_range(0..SUPPORTED_OPERATIONS.len());
        SUPPORTED_OPERATIONS[idx]
    }
    ...
    fn name(&self) -> &'static str {
        match self {
            ...
            // Policy operations - Warehouse
            ChaosOperation::GetWarehousePolicy => "GetWhPolicy",
            ChaosOperation::PutWarehousePolicy => "PutWhPolicy",
            ChaosOperation::DeleteWarehousePolicy => "DelWhPolicy",
            // Policy operations - Table
            ChaosOperation::GetTablePolicy => "GetTblPolicy",
            ChaosOperation::PutTablePolicy => "PutTblPolicy",
            ChaosOperation::DeleteTablePolicy => "DelTblPolicy",
            // Encryption operations - Warehouse
            ChaosOperation::GetWarehouseEncryption => "GetWhEncrypt",
            ChaosOperation::PutWarehouseEncryption => "PutWhEncrypt",
            ChaosOperation::DeleteWarehouseEncryption => "DelWhEncrypt",
            // Encryption operations - Table
            ChaosOperation::GetTableEncryption => "GetTblEncrypt",
            ChaosOperation::PutTableEncryption => "PutTblEncrypt",
            ChaosOperation::DeleteTableEncryption => "DelTblEncrypt",
            // Maintenance operations - Warehouse
            ChaosOperation::GetWarehouseMaintenance => "GetWhMaint",
            ChaosOperation::PutWarehouseMaintenance => "PutWhMaint",
            // Maintenance operations - Table
            ChaosOperation::GetTableMaintenance => "GetTblMaint",
            ChaosOperation::PutTableMaintenance => "PutTblMaint",
            ChaosOperation::GetTableMaintenanceJobStatus => "GetTblMaintJob",
            // Replication operations - Warehouse
            ChaosOperation::GetWarehouseReplication => "GetWhRepl",
            ChaosOperation::PutWarehouseReplication => "PutWhRepl",
            ChaosOperation::DeleteWarehouseReplication => "DelWhRepl",
            // Replication operations - Table
            ChaosOperation::GetTableReplication => "GetTblRepl",
            ChaosOperation::PutTableReplication => "PutTblRepl",
            ChaosOperation::DeleteTableReplication => "DelTblRepl",
            ChaosOperation::GetTableReplicationStatus => "GetTblReplStat",
            // Storage class operations
            ChaosOperation::GetWarehouseStorageClass => "GetWhStorage",
            ChaosOperation::PutWarehouseStorageClass => "PutWhStorage",
            ChaosOperation::GetTableStorageClass => "GetTblStorage",
            // Metrics operations - Warehouse
            ChaosOperation::GetWarehouseMetrics => "GetWhMetrics",
            ChaosOperation::PutWarehouseMetrics => "PutWhMetrics",
            ChaosOperation::DeleteWarehouseMetrics => "DelWhMetrics",
            // Expiration operations - Table
            ChaosOperation::GetTableExpiration => "GetTblExpir",
            ChaosOperation::PutTableExpiration => "PutTblExpir",
            ChaosOperation::GetTableExpirationJobStatus => "GetTblExpirJob",
            // Tagging operations
            ChaosOperation::TagResource => "TagResource",
            ChaosOperation::UntagResource => "UntagResource",
            ChaosOperation::ListTagsForResource => "ListTags",
            // Direct scan execution
            ChaosOperation::ExecuteTableScan => "ExecTableScan",
        }
    }
}
...
struct ChaosMetrics {
    ...
    // Policy operations - Warehouse (3)
    get_warehouse_policy: AtomicU64,
    put_warehouse_policy: AtomicU64,
    delete_warehouse_policy: AtomicU64,
    // Policy operations - Table (3)
    get_table_policy: AtomicU64,
    put_table_policy: AtomicU64,
    delete_table_policy: AtomicU64,
    // Encryption operations - Warehouse (3)
    get_warehouse_encryption: AtomicU64,
    put_warehouse_encryption: AtomicU64,
    delete_warehouse_encryption: AtomicU64,
    // Encryption operations - Table (3)
    get_table_encryption: AtomicU64,
    put_table_encryption: AtomicU64,
    delete_table_encryption: AtomicU64,
    // Maintenance operations - Warehouse (2)
    get_warehouse_maintenance: AtomicU64,
    put_warehouse_maintenance: AtomicU64,
    // Maintenance operations - Table (3)
    get_table_maintenance: AtomicU64,
    put_table_maintenance: AtomicU64,
    get_table_maintenance_job_status: AtomicU64,
    // Replication operations - Warehouse (3)
    get_warehouse_replication: AtomicU64,
    put_warehouse_replication: AtomicU64,
    delete_warehouse_replication: AtomicU64,
    // Replication operations - Table (4)
    get_table_replication: AtomicU64,
    put_table_replication: AtomicU64,
    delete_table_replication: AtomicU64,
    get_table_replication_status: AtomicU64,
    // Storage class operations (3)
    get_warehouse_storage_class: AtomicU64,
    put_warehouse_storage_class: AtomicU64,
    get_table_storage_class: AtomicU64,
    // Metrics operations - Warehouse (3)
    get_warehouse_metrics: AtomicU64,
    put_warehouse_metrics: AtomicU64,
    delete_warehouse_metrics: AtomicU64,
    // Expiration operations - Table (3)
    get_table_expiration: AtomicU64,
    put_table_expiration: AtomicU64,
    get_table_expiration_job_status: AtomicU64,
    // Tagging operations (3)
    tag_resource: AtomicU64,
    untag_resource: AtomicU64,
    list_tags_for_resource: AtomicU64,
    // Direct scan execution (1)
    execute_table_scan: AtomicU64,
    // Error tracking
...
impl ChaosMetrics {
    fn new() -> Self {
        Self {
            ...
            // Policy operations - Warehouse (3)
            get_warehouse_policy: AtomicU64::new(0),
            put_warehouse_policy: AtomicU64::new(0),
            delete_warehouse_policy: AtomicU64::new(0),
            // Policy operations - Table (3)
            get_table_policy: AtomicU64::new(0),
            put_table_policy: AtomicU64::new(0),
            delete_table_policy: AtomicU64::new(0),
            // Encryption operations - Warehouse (3)
            get_warehouse_encryption: AtomicU64::new(0),
            put_warehouse_encryption: AtomicU64::new(0),
            delete_warehouse_encryption: AtomicU64::new(0),
            // Encryption operations - Table (3)
            get_table_encryption: AtomicU64::new(0),
            put_table_encryption: AtomicU64::new(0),
            delete_table_encryption: AtomicU64::new(0),
            // Maintenance operations - Warehouse (2)
            get_warehouse_maintenance: AtomicU64::new(0),
            put_warehouse_maintenance: AtomicU64::new(0),
            // Maintenance operations - Table (3)
            get_table_maintenance: AtomicU64::new(0),
            put_table_maintenance: AtomicU64::new(0),
            get_table_maintenance_job_status: AtomicU64::new(0),
            // Replication operations - Warehouse (3)
            get_warehouse_replication: AtomicU64::new(0),
            put_warehouse_replication: AtomicU64::new(0),
            delete_warehouse_replication: AtomicU64::new(0),
            // Replication operations - Table (4)
            get_table_replication: AtomicU64::new(0),
            put_table_replication: AtomicU64::new(0),
            delete_table_replication: AtomicU64::new(0),
            get_table_replication_status: AtomicU64::new(0),
            // Storage class operations (3)
            get_warehouse_storage_class: AtomicU64::new(0),
            put_warehouse_storage_class: AtomicU64::new(0),
            get_table_storage_class: AtomicU64::new(0),
            // Metrics operations - Warehouse (3)
            get_warehouse_metrics: AtomicU64::new(0),
            put_warehouse_metrics: AtomicU64::new(0),
            delete_warehouse_metrics: AtomicU64::new(0),
            // Expiration operations - Table (3)
            get_table_expiration: AtomicU64::new(0),
            put_table_expiration: AtomicU64::new(0),
            get_table_expiration_job_status: AtomicU64::new(0),
            // Tagging operations (3)
            tag_resource: AtomicU64::new(0),
            untag_resource: AtomicU64::new(0),
            list_tags_for_resource: AtomicU64::new(0),
            // Direct scan execution (1)
            execute_table_scan: AtomicU64::new(0),
            // Error tracking
    ...
    fn total_ops(&self) -> u64 {
        ...
        self.create_warehouse.load(Ordering::Relaxed)
            ...
            // Policy operations - Warehouse (3)
            + self.get_warehouse_policy.load(Ordering::Relaxed)
            + self.put_warehouse_policy.load(Ordering::Relaxed)
            + self.delete_warehouse_policy.load(Ordering::Relaxed)
            // Policy operations - Table (3)
            + self.get_table_policy.load(Ordering::Relaxed)
            + self.put_table_policy.load(Ordering::Relaxed)
            + self.delete_table_policy.load(Ordering::Relaxed)
            // Encryption operations - Warehouse (3)
            + self.get_warehouse_encryption.load(Ordering::Relaxed)
            + self.put_warehouse_encryption.load(Ordering::Relaxed)
            + self.delete_warehouse_encryption.load(Ordering::Relaxed)
            // Encryption operations - Table (3)
            + self.get_table_encryption.load(Ordering::Relaxed)
            + self.put_table_encryption.load(Ordering::Relaxed)
            + self.delete_table_encryption.load(Ordering::Relaxed)
            // Maintenance operations - Warehouse (2)
            + self.get_warehouse_maintenance.load(Ordering::Relaxed)
            + self.put_warehouse_maintenance.load(Ordering::Relaxed)
            // Maintenance operations - Table (3)
            + self.get_table_maintenance.load(Ordering::Relaxed)
            + self.put_table_maintenance.load(Ordering::Relaxed)
            + self.get_table_maintenance_job_status.load(Ordering::Relaxed)
            // Replication operations - Warehouse (3)
            + self.get_warehouse_replication.load(Ordering::Relaxed)
            + self.put_warehouse_replication.load(Ordering::Relaxed)
            + self.delete_warehouse_replication.load(Ordering::Relaxed)
            // Replication operations - Table (4)
            + self.get_table_replication.load(Ordering::Relaxed)
            + self.put_table_replication.load(Ordering::Relaxed)
            + self.delete_table_replication.load(Ordering::Relaxed)
            + self.get_table_replication_status.load(Ordering::Relaxed)
            // Storage class operations (3)
            + self.get_warehouse_storage_class.load(Ordering::Relaxed)
            + self.put_warehouse_storage_class.load(Ordering::Relaxed)
            + self.get_table_storage_class.load(Ordering::Relaxed)
            // Metrics operations - Warehouse (3)
            + self.get_warehouse_metrics.load(Ordering::Relaxed)
            + self.put_warehouse_metrics.load(Ordering::Relaxed)
            + self.delete_warehouse_metrics.load(Ordering::Relaxed)
            // Expiration operations - Table (3)
            + self.get_table_expiration.load(Ordering::Relaxed)
            + self.put_table_expiration.load(Ordering::Relaxed)
            + self.get_table_expiration_job_status.load(Ordering::Relaxed)
            // Tagging operations (3)
            + self.tag_resource.load(Ordering::Relaxed)
            + self.untag_resource.load(Ordering::Relaxed)
            + self.list_tags_for_resource.load(Ordering::Relaxed)
            // Direct scan execution (1)
            + self.execute_table_scan.load(Ordering::Relaxed)
    }
    ...
    fn record(&self, op: ChaosOperation) {
        match op {
            ...
            // Policy operations - Warehouse
            ChaosOperation::GetWarehousePolicy => {
                self.get_warehouse_policy.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::PutWarehousePolicy => {
                self.put_warehouse_policy.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::DeleteWarehousePolicy => {
                self.delete_warehouse_policy.fetch_add(1, Ordering::Relaxed)
            }
            // Policy operations - Table
            ChaosOperation::GetTablePolicy => self.get_table_policy.fetch_add(1, Ordering::Relaxed),
            ChaosOperation::PutTablePolicy => self.put_table_policy.fetch_add(1, Ordering::Relaxed),
            ChaosOperation::DeleteTablePolicy => {
                self.delete_table_policy.fetch_add(1, Ordering::Relaxed)
            }
            // Encryption operations - Warehouse
            ChaosOperation::GetWarehouseEncryption => self
                .get_warehouse_encryption
                .fetch_add(1, Ordering::Relaxed),
            ChaosOperation::PutWarehouseEncryption => self
                .put_warehouse_encryption
                .fetch_add(1, Ordering::Relaxed),
            ChaosOperation::DeleteWarehouseEncryption => self
                .delete_warehouse_encryption
                .fetch_add(1, Ordering::Relaxed),
            // Encryption operations - Table
            ChaosOperation::GetTableEncryption => {
                self.get_table_encryption.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::PutTableEncryption => {
                self.put_table_encryption.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::DeleteTableEncryption => {
                self.delete_table_encryption.fetch_add(1, Ordering::Relaxed)
            }
            // Maintenance operations - Warehouse
            ChaosOperation::GetWarehouseMaintenance => self
                .get_warehouse_maintenance
                .fetch_add(1, Ordering::Relaxed),
            ChaosOperation::PutWarehouseMaintenance => self
                .put_warehouse_maintenance
                .fetch_add(1, Ordering::Relaxed),
            // Maintenance operations - Table
            ChaosOperation::GetTableMaintenance => {
                self.get_table_maintenance.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::PutTableMaintenance => {
                self.put_table_maintenance.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::GetTableMaintenanceJobStatus => self
                .get_table_maintenance_job_status
                .fetch_add(1, Ordering::Relaxed),
            // Replication operations - Warehouse
            ChaosOperation::GetWarehouseReplication => self
                .get_warehouse_replication
                .fetch_add(1, Ordering::Relaxed),
            ChaosOperation::PutWarehouseReplication => self
                .put_warehouse_replication
                .fetch_add(1, Ordering::Relaxed),
            ChaosOperation::DeleteWarehouseReplication => self
                .delete_warehouse_replication
                .fetch_add(1, Ordering::Relaxed),
            // Replication operations - Table
            ChaosOperation::GetTableReplication => {
                self.get_table_replication.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::PutTableReplication => {
                self.put_table_replication.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::DeleteTableReplication => self
                .delete_table_replication
                .fetch_add(1, Ordering::Relaxed),
            ChaosOperation::GetTableReplicationStatus => self
                .get_table_replication_status
                .fetch_add(1, Ordering::Relaxed),
            // Storage class operations
            ChaosOperation::GetWarehouseStorageClass => self
                .get_warehouse_storage_class
                .fetch_add(1, Ordering::Relaxed),
            ChaosOperation::PutWarehouseStorageClass => self
                .put_warehouse_storage_class
                .fetch_add(1, Ordering::Relaxed),
            ChaosOperation::GetTableStorageClass => {
                self.get_table_storage_class.fetch_add(1, Ordering::Relaxed)
            }
            // Metrics operations - Warehouse
            ChaosOperation::GetWarehouseMetrics => {
                self.get_warehouse_metrics.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::PutWarehouseMetrics => {
                self.put_warehouse_metrics.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::DeleteWarehouseMetrics => self
                .delete_warehouse_metrics
                .fetch_add(1, Ordering::Relaxed),
            // Expiration operations - Table
            ChaosOperation::GetTableExpiration => {
                self.get_table_expiration.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::PutTableExpiration => {
                self.put_table_expiration.fetch_add(1, Ordering::Relaxed)
            }
            ChaosOperation::GetTableExpirationJobStatus => self
                .get_table_expiration_job_status
                .fetch_add(1, Ordering::Relaxed),
            // Tagging operations
            ChaosOperation::TagResource => self.tag_resource.fetch_add(1, Ordering::Relaxed),
            ChaosOperation::UntagResource => self.untag_resource.fetch_add(1, Ordering::Relaxed),
            ChaosOperation::ListTagsForResource => {
                self.list_tags_for_resource.fetch_add(1, Ordering::Relaxed)
            }
            // Direct scan execution
            ChaosOperation::ExecuteTableScan => {
                self.execute_table_scan.fetch_add(1, Ordering::Relaxed)
            }
        };
    }
        ...
        match operation {
            ChaosOperation::CreateWarehouse => {
                ...
                if let Ok(warehouse) = WarehouseName::try_from(wh_name.as_str())
                    && let Ok(builder) = tables.create_warehouse(warehouse)
                {
            ...
            ChaosOperation::DeleteWarehouse => {
                if let Some(wh) = state.random_warehouse()
                    ...
                    && let Ok(builder) = tables.delete_warehouse(warehouse)
                {
                    match builder.build().send().await {
                        Ok(_) => {
            ...
            ChaosOperation::CreateNamespace => {
                if let Some(wh) = state.random_warehouse() {
                    ...
                    ) && let Ok(builder) = tables.create_namespace(warehouse, ns)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
            ...
            ChaosOperation::DeleteNamespace => {
                if let Some((wh, ns)) = state.random_namespace() {
                    ...
                    ) && let Ok(builder) = tables.delete_namespace(warehouse, namespace)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
            ...
            ChaosOperation::CreateTable => {
                if let Some((wh, ns)) = state.random_namespace() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                        ...
                        tables.create_table(warehouse, namespace, table_name, schema.clone())
                    {
                        match builder.build().send().await {
                            Ok(_) => {
            ...
            ChaosOperation::DeleteTable => {
                if let Some((wh, ns, tbl)) = state.random_table() {
                    ...
                    ) && let Ok(builder) = tables.delete_table(warehouse, namespace, table_name)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
            ...
            ChaosOperation::RenameTable => {
                if let Some((wh, ns, tbl)) = state.random_table() {
                    ...
                    ) && let Ok(builder) = tables.rename_table(
                        warehouse,
                        namespace.clone(),
                        table_name,
                        namespace,
            ...
            ChaosOperation::LoadTable => {
                if let Some((wh, ns, tbl)) = state.random_table() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                        ...
                        Namespace::try_from(ns.clone()),
                        TableName::try_from(tbl.as_str()),
                    ) && let Ok(builder) = tables.load_table(warehouse, namespace, table_name)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::ListNamespaces => {
                if let Some(wh) = state.random_warehouse()
                    && let Ok(warehouse) = WarehouseName::try_from(wh.as_str())
                    && let Ok(builder) = tables.list_namespaces(warehouse)
                {
                    match builder.build().send().await {
                        Ok(_) => {
                            success = true;
            ...
            ChaosOperation::ListTables => {
                if let Some((wh, ns)) = state.random_namespace() {
                    ...
                    if let (Ok(warehouse), Ok(namespace)) = (
                        WarehouseName::try_from(wh.as_str()),
                        Namespace::try_from(ns.clone()),
                    ) && let Ok(builder) = tables.list_tables(warehouse, namespace)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::GetWarehouse => {
                if let Some(wh) = state.random_warehouse()
                    && let Ok(warehouse) = WarehouseName::try_from(wh.as_str())
                    && let Ok(builder) = tables.get_warehouse(warehouse)
                {
                    match builder.build().send().await {
                        Ok(_) => {
                            success = true;
            ...
            ChaosOperation::GetNamespace => {
                if let Some((wh, ns)) = state.random_namespace() {
                    ...
                    if let (Ok(warehouse), Ok(namespace)) = (
                        WarehouseName::try_from(wh.as_str()),
                        Namespace::try_from(ns.clone()),
                    ) && let Ok(builder) = tables.get_namespace(warehouse, namespace)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::NamespaceExists => {
                if let Some((wh, ns)) = state.random_namespace() {
                    ...
                    if let (Ok(warehouse), Ok(namespace)) = (
                        WarehouseName::try_from(wh.as_str()),
                        Namespace::try_from(ns.clone()),
                    ) && let Ok(builder) = tables.namespace_exists(warehouse, namespace)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::TableExists => {
                if let Some((wh, ns, tbl)) = state.random_table() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                        ...
                        Namespace::try_from(ns.clone()),
                        TableName::try_from(tbl.as_str()),
                    ) && let Ok(builder) = tables.table_exists(warehouse, namespace, table_name)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::CommitTable => {
                ...
                if let Some((wh, ns, tbl)) = state.random_table() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                        ...
                        tables.load_table(warehouse.clone(), namespace.clone(), table_name.clone())
                    {
                        // First load the table to get current metadata
                        match load_builder.build().send().await {
                            Ok(load_resp) => {
                                // Get the table result which contains metadata
                                match load_resp.table_result() {
                                    Ok(_result) => {
                                        // Now try to commit (no-op update)
                                        if let Ok(commit_builder) =
                                            tables.commit_table(warehouse, namespace, table_name)
                                        {
                                            match commit_builder.build().send().await {
                                                Ok(_) => {
                                                    success = true;
                                                }
                                                Err(e) => {
            ...
            ChaosOperation::RegisterTable => {
                ...
                if let Some((wh, ns, tbl)) = state.random_table() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                        ...
                        tables.load_table(warehouse.clone(), namespace.clone(), table_name)
                    {
                                ...
                                if let Ok(result) = load_resp.table_result() {
                                    ...
                                    if let Some(metadata_loc) = result.metadata_location {
                                            ...
                                            TableName::try_from(new_tbl_name.as_str())
                                        {
                                            ...
                                            if let Ok(reg_builder) = tables.register_table(
                                                warehouse,
                                                namespace.clone(),
                                                new_table_name,
                                                metadata_loc,
                                            ) {
                                                match reg_builder.build().send().await {
                                                    Ok(_) => {
                    ...
                    if wh1 == wh2 {
                        ...
                        if let (
                            ...
                            let load1 = tables
                                ...
                                .ok()
                                .map(|b| async move { b.build().send().await });
                            let load2 = tables
                                .load_table(
                                    warehouse.clone(),
                                    namespace2.clone(),
                                ...
                                .ok()
                                .map(|b| async move { b.build().send().await });
                            let (load1, load2) = match (load1, load2) {
                                (Some(f1), Some(f2)) => (f1.await, f2.await),
                                _ => continue,
                            };
                            ...
                            match (load1, load2) {
                                (Ok(resp1), Ok(resp2)) => {
                                        ...
                                        (resp1.table_result(), resp2.table_result())
                                    {
                                        ...
                                        if let Ok(txn_builder) = tables
                                            .commit_multi_table_transaction(warehouse, changes)
                                        {
                                            match txn_builder.build().send().await {
                                                Ok(_) => {
                                                    success = true;
                                                }
                                                Err(e) => {
            ...
            ChaosOperation::GetConfig => {
                if let Some(wh) = state.random_warehouse()
                    && let Ok(warehouse) = WarehouseName::try_from(wh.as_str())
                    && let Ok(builder) = tables.get_config(warehouse)
                {
                    match builder.build().send().await {
                        Ok(_) => {
                            success = true;
            ...
            ChaosOperation::TableMetrics => {
                if let Some((wh, ns, tbl)) = state.random_table() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                        ...
                        Namespace::try_from(ns.clone()),
                        TableName::try_from(tbl.as_str()),
                    ) && let Ok(builder) = tables.table_metrics(warehouse, namespace, table_name)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::UpdateNamespaceProperties => {
                if let Some((wh, ns)) = state.random_namespace() {
                    ...
                    if let (Ok(warehouse), Ok(namespace)) = (
                        ...
                        tables.update_namespace_properties(warehouse, namespace)
                    {
                        ...
                        match builder.updates(updates).build().unwrap().send().await {
                            Ok(_) => {
                                success = true;
                            }
                            Err(e) => {
            ...
            ChaosOperation::LoadTableCredentials => {
                if let Some((wh, ns, tbl)) = state.random_table() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                        ...
                        tables.load_table_credentials(warehouse, namespace, table_name)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::CreateView => {
                if let Some((wh, ns)) = state.random_namespace() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(view_name_wrapped)) = (
                        ...
                        if let Ok(builder) = tables.create_view(
                            warehouse,
                            namespace.clone(),
                            view_name_wrapped,
                            schema.clone(),
            ...
            ChaosOperation::ListViews => {
                if let Some((wh, ns)) = state.random_namespace() {
                    ...
                    if let (Ok(warehouse), Ok(namespace)) = (
                        WarehouseName::try_from(wh.as_str()),
                        Namespace::try_from(ns.clone()),
                    ) && let Ok(builder) = tables.list_views(warehouse, namespace)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::LoadView => {
                if let Some((wh, ns, view)) = state.random_view() {
                    ...
                    ) && let Ok(builder) = tables.load_view(warehouse, namespace, view_name)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::ReplaceView => {
                if let Some((wh, ns, view)) = state.random_view() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(view_name)) = (
                        ...
                        Namespace::try_from(ns.clone()),
                        ViewName::try_from(view.as_str()),
                    ) && let Ok(builder) = tables.replace_view(warehouse, namespace, view_name)
                    {
            ...
            ChaosOperation::DropView => {
                if let Some((wh, ns, view)) = state.random_view() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(view_name)) = (
                        ...
                        tables.drop_view(warehouse, namespace.clone(), view_name)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
            ...
            ChaosOperation::ViewExists => {
                if let Some((wh, ns, view)) = state.random_view() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(view_name)) = (
                        ...
                        Namespace::try_from(ns.clone()),
                        ViewName::try_from(view.as_str()),
                    ) && let Ok(builder) = tables.view_exists(warehouse, namespace, view_name)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::RenameView => {
                if let Some((wh, ns, view)) = state.random_view() {
                    ...
                    ) && let Ok(builder) = tables.rename_view(
                        warehouse,
                        namespace.clone(),
                        view_name,
                        namespace.clone(),
            ...
            ChaosOperation::PlanTableScan => {
                if let Some((wh, ns, tbl)) = state.random_table() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                        ...
                        tables.plan_table_scan(warehouse, namespace.clone(), table_name)
                    {
                        match builder.build().send().await {
                            Ok(resp) => {
                                success = true;
            ...
            ChaosOperation::FetchPlanningResult => {
                if let Some((wh, ns, tbl, plan_id_str)) = state.random_plan_id() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name), Ok(plan_id)) = (
                        ...
                        tables.fetch_planning_result(warehouse, namespace, table_name, plan_id)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::FetchScanTasks => {
                if let Some((wh, ns, tbl, plan_id)) = state.random_plan_id() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                        ...
                        tables.fetch_scan_tasks(warehouse, namespace, table_name, plan_task)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            ChaosOperation::CancelPlanning => {
                if let Some((wh, ns, tbl, plan_id_str)) = state.random_plan_id() {
                    ...
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name), Ok(plan_id)) = (
                        ...
                        tables.cancel_planning(warehouse, namespace.clone(), table_name, plan_id)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            // Execute table scan (synchronous scan execution)
            ChaosOperation::ExecuteTableScan => {
                if let Some((wh, ns, tbl)) = state.random_table() {
                    let resource = format!("{}/{}/{}", wh, ns.join("/"), tbl);
                    if let (Ok(warehouse), Ok(namespace), Ok(table_name)) = (
                        ...
                        tables.execute_table_scan(warehouse, namespace, table_name)
                    {
                        match builder.build().send().await {
                            Ok(_) => {
                                success = true;
            ...
            // Policy operations - Warehouse (not yet supported)
            ChaosOperation::GetWarehousePolicy
            | ChaosOperation::PutWarehousePolicy
            | ChaosOperation::DeleteWarehousePolicy => {
                // Not yet supported by server - skip operation
            }

            // Policy operations - Table (not yet supported)
            ChaosOperation::GetTablePolicy
            | ChaosOperation::PutTablePolicy
            | ChaosOperation::DeleteTablePolicy => {
                // Not yet supported by server - skip operation
            ...
            // Encryption operations - Warehouse (not yet supported)
            ChaosOperation::GetWarehouseEncryption
            | ChaosOperation::PutWarehouseEncryption
            | ChaosOperation::DeleteWarehouseEncryption => {
                // Not yet supported by server - skip operation
            ...
            // Encryption operations - Table (not yet supported)
            ChaosOperation::GetTableEncryption
            | ChaosOperation::PutTableEncryption
            | ChaosOperation::DeleteTableEncryption => {
                // Not yet supported by server - skip operation
            ...
            // Maintenance operations - Warehouse (not yet supported)
            ChaosOperation::GetWarehouseMaintenance | ChaosOperation::PutWarehouseMaintenance => {
                // Not yet supported by server - skip operation
            }

            // Maintenance operations - Table (not yet supported)
            ChaosOperation::GetTableMaintenance
            | ChaosOperation::PutTableMaintenance
            | ChaosOperation::GetTableMaintenanceJobStatus => {
                // Not yet supported by server - skip operation
            ...
            // Replication operations - Warehouse (not yet supported)
            ChaosOperation::GetWarehouseReplication
            | ChaosOperation::PutWarehouseReplication
            | ChaosOperation::DeleteWarehouseReplication => {
                // Not yet supported by server - skip operation
            ...
            // Replication operations - Table (not yet supported)
            ChaosOperation::GetTableReplication
            | ChaosOperation::PutTableReplication
            | ChaosOperation::DeleteTableReplication
            | ChaosOperation::GetTableReplicationStatus => {
            ...
            // Storage class operations (not yet supported)
            ChaosOperation::GetWarehouseStorageClass
            | ChaosOperation::PutWarehouseStorageClass
            | ChaosOperation::GetTableStorageClass => {
                // Not yet supported by server - skip operation
            ...
            // Metrics operations - Warehouse (not yet supported)
            ChaosOperation::GetWarehouseMetrics
            | ChaosOperation::PutWarehouseMetrics
            | ChaosOperation::DeleteWarehouseMetrics => {
                // Not yet supported by server - skip operation
            ...
            // Expiration operations - Table (not yet supported)
            ChaosOperation::GetTableExpiration
            | ChaosOperation::PutTableExpiration
            | ChaosOperation::GetTableExpirationJobStatus => {
                // Not yet supported by server - skip operation
            ...
            // Tagging operations (not yet supported)
            ChaosOperation::TagResource
            | ChaosOperation::UntagResource
            | ChaosOperation::ListTagsForResource => {
                // Not yet supported by server - skip operation
            ...
            && let Ok(builder) = tables.create_warehouse(warehouse)
        {
            ...
            for ns_idx in 0..NUM_INITIAL_NAMESPACES_PER_WH {
                ...
                if let (Ok(warehouse), Ok(ns)) = (
                    ...
                    Namespace::try_from(namespace.clone()),
                ) && let Ok(builder) = tables.create_namespace(warehouse.clone(), ns)
                {
                    match builder.build().send().await {
                        Ok(_) => {
                    ...
                    for tbl_idx in 0..NUM_INITIAL_TABLES_PER_NS {
                        ...
                        if let (Ok(warehouse), Ok(ns), Ok(table_name)) = (
                            ...
                            tables.create_table(warehouse, ns, table_name, schema.clone())
                        {
                            match builder.build().send().await {
                                Ok(_) => {

...
async fn client_task(
    ...
    while !stop_signal.load(Ordering::Relaxed) {
        ...
        let success = if operation < 4 {
            ...
            // 30% list_tables
            match tables.list_tables(warehouse.clone(), ns.clone()) {
                Ok(builder) => builder.build().send().await.is_ok(),
                Err(_) => false,
            }
        } else if operation < 9 {
            // 20% list_namespaces
            match tables.list_namespaces(warehouse.clone()) {
                Ok(builder) => builder.build().send().await.is_ok(),
                Err(_) => false,
            }
        } else {
            // 10% get_warehouse
            match tables.get_warehouse(warehouse.clone()) {
                Ok(builder) => builder.build().send().await.is_ok(),
                Err(_) => false,
            }
        };
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Create warehouse
    match tables.create_warehouse(warehouse.clone()) {
        ...
        Err(e) => println!("  Validation error for warehouse: {}", e),
    }
    ...
    for ns_idx in 0..NUM_NAMESPACES {
        ...
        match tables.create_namespace(warehouse.clone(), namespace.clone()) {
            Ok(builder) => match builder.build().send().await {
                Ok(_) => println!("  Created namespace: {}", ns_name),
                Err(e) => println!("  Note: Namespace may already exist: {}", e),
            },
            Err(e) => println!("  Validation error for namespace: {}", e),
        }
        ...
        for tbl_idx in 0..NUM_TABLES_PER_NS {
            ...
            match tables.create_table(
                ...
                Err(e) => println!("    Validation error for table: {}", e),
            }
